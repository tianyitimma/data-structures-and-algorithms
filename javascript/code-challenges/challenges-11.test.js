...
const totalSum = (input) => {
  let sum = 0;
  input.map(item => item.map(num => sum += num));
  return sum;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let power = input.map(item => item.filter(num => (num%5 === 0) && (typeof num === 'number')));
  return power.map(item => item.map(num => Math.pow(2,num)));
};
...
let findMaleAndFemale = (data) => {
  let maleAndFemale = [];
  let result = '';
  data.filter(obj => obj.gender === 'male' || obj.gender === 'female').map(item => maleAndFemale.push(item.name));

  for(let i=0; i < maleAndFemale.length - 1; i++){
    result += `${maleAndFemale[i]} and `;
  }

  return result += `${maleAndFemale[maleAndFemale.length-1]}`;
