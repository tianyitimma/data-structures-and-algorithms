...
const toLastNames = people => {
  let newPeople = people.map(person => `${person.firstName} ${person.lastName}`);
  return newPeople;
};
...
const countNumberOfElements = (arr) => {
  let count = arr.reduce((acc, item, idx) => acc = idx + 1 , 0);
  return count;
};
...
const returnNames = (arr) => {
  let names = arr.reduce((acc, person) => {
...
const reversedString = (str) => {
  let arr = str.split('');
  let revString = arr.reduce((acc, current) => {
    return current + acc;
  }, '');
  return revString;
};
